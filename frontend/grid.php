...
if (isset($_GET['sort'])) { //search term
    $sort_what = $conn->real_escape_string($_GET['sort']);
    switch($sort_what){
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $grid_str=$grid_str."</div>
                ...
                $vol_text=preg_split('/[0-9,.]*/',$row["volume"],2)[1];
                $vol_num= str_replace($vol_text,"",$row["volume"]);
                if ($vol_num=="")
                {
                    $vol_num=1;
                }
                ...
                <table align=\"center\">
                    <tr>
                      <td style=\"padding: 0 5px 0 5px\">
                        <span class=\"label measurments label-default\"><span style=\"font-size: 18px;\">".$vol_num."</span><br>".$vol_text."</span>
